...
const { extractActivityLogInfo, generatePrivateKey } = requireRoot('lib/utils')
...
const SecurityAccount = keystone.list('SecurityAccount')
const SecuritySocial = keystone.list('SecuritySocial')
const SecurityClient = keystone.list('SecurityClient')
...
class SecurityService {
  ...
  async findSocial ({ socialNetwork, socialId }) {
    return SecuritySocial.model
      .findOne({ socialNetwork, socialId })
      .populate('user')
  }

  async findApiClient ({ id, secret }) {
    return SecurityClient.model
      .findOne({ _id: id, secret })
      .populate({
        path: 'agent',
        populate: { path: 'user' }
      })
  ...
  async selectAccounts (user) {
    const accounts = await SecurityAccount.model.find({user: user._id})
    return accounts.map(AccountModel.fromMongo)
  }

  async selectAccount (user, { type, address }) {
    return SecurityAccount.model
      .findOne({
        user: user._id,
        type,
        address
  ...
  async createAccount (user, createModel) {
    assert(createModel instanceof CreateAccountModel)
    const account = await SecurityAccount.model.create({
      user: user._id,
      type: createModel.type,
      privateKey: createModel.privateKey
    })
    return AccountModel.fromMongo(account)
  }
  ...
  _checkAddresses(addresses) {
    return addresses.filter(addr => addr.type === 'ethereum-public-key').length > 0;
  }

  async upsertAddresses ({user, addresses}) {
    if (!this._checkAddresses(addresses))
      throw new Error('Addresses not valid - include ethereum-public-key');
    await Promise.all(
  ...
  async upsertSocial ({ purpose, socialNetwork, socialId, accessToken, refreshToken }) {
    let social = await SecuritySocial.model
      .findOne({
        purpose,
        socialId,
        socialNetwork
    ...
    if (social) {
      ...
      const user = await SecurityUser.model.create({
        name: `${purpose}:${socialNetwork}:${socialId}`
      })
      social = await SecuritySocial.model
        .create({
          user: user._id,
          purpose: purpose,
          socialId,
  ...
  async upsertAgent (user, createModel) {
    assert(createModel instanceof CreateAgentModel)
    let agent = await SecurityAgent.model.findOne({
      user: user._id,
      secret: null,
      contract: createModel.contract.toLowerCase(),
      principal: createModel.principal.toLowerCase(),
      purpose: createModel.purpose
    })
    if (agent) {
      agent.secret = createModel.secret
      await agent.save()
    } else {
      agent = await SecurityAgent.model.create({
        user: user._id,
  ...
  async createApiClient (user, createModel) {
    assert(createModel instanceof CreateApiClientModel)
    let agent = await SecurityAgent.model.findOne({
      user: user._id.toString(),
      purpose: createModel.purpose,
      contract: createModel.contract.toLowerCase(),
    ...
    const client = await SecurityClient.model.create({
      agent: agent._id,
      privateKey: generatePrivateKey(),
      isReadAllowed: createModel.isReadAllowed,
      isMarketAllowed: createModel.isMarketAllowed,
      isLimitAllowed: createModel.isLimitAllowed,
      isWithdrawAllowed: createModel.isWithdrawAllowed
    })
    return ApiClientModel.fromMongo(client)
  }

  async removeApiClient (user, { id }) {
    const client = await SecurityClient.model
      .findOne({
        _id: id
      })
      .populate('agent')
    if (!client || !client.agent || client.agent.user.toString() !== user._id.toString()) {
