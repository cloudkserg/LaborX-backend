...
const { securityService, signerService } = requireRoot('lib/services')
const { asyncHandler, authenticate, AUTH_TYPE_API_KEYS } = require('../middleware')
const {
...
router.get('/me/accounts', authenticate(), asyncHandler(async (req, res) => {
  const { user } = req.securityContext
  const accounts = await securityService.selectAccounts(user)
  res.send(accounts)
}))

router.get('/me/accounts/t/:type/a/:address', authenticate(), asyncHandler(async (req, res) => {
  const { type, address } = req.params
  const { user } = req.securityContext
  const account = await securityService.selectAccount(user, {
    type,
    address
  })
  res.send({
...
router.post('/me/accounts', authenticate(), asyncHandler(async (req, res) => {
  const { user } = req.securityContext
  const account = await securityService.createAccount(
    user,
    CreateAccountModel.fromJson(req.body)
  )
  res.send(account)
}))
...
router.post('/me/agents', authenticate(), asyncHandler(async (req, res) => {
  const { user } = req.securityContext
  await securityService.upsertAgent(
    user,
...
router.post('/me/api-clients', authenticate(), asyncHandler(async (req, res) => {
  try {
    const { user } = req.securityContext
    await securityService.createApiClient(
      user,
      CreateApiClientModel.fromJson(req.body)
    )
    const profile = await securityService.selectProfile(user)
    res.send(profile)
  } catch (e) {
...
router.delete('/me/api-clients/i/:id', authenticate(), asyncHandler(async (req, res) => {
  try {
    const { user } = req.securityContext
    await securityService.removeApiClient(
      user,
      { id: req.params.id }
    )
    const profile = await securityService.selectProfile(user)
...
router.post('/sign/tx', authenticate(), asyncHandler(async (req, res) => {
  const { user } = req.securityContext
  const { type, txData, address } = req.body
  const account = await securityService.selectAccount(user, {
    type,
    address
  })
  const { privateKey } = account
  const signed = await signerService.signTransaction(privateKey, txData)
  res.send(signed)
}))

router.post('/sign/data', authenticate(), asyncHandler(async (req, res) => {
  const { user } = req.securityContext
  ...
  const { privateKey } = account
  const signed = await signerService.signData(privateKey, data)
  res.send(signed)
}))

router.post('/apikeys/sign/data', authenticate({ types: [AUTH_TYPE_API_KEYS] }), asyncHandler(async (req, res) => {
  const { apiClient } = req.securityContext
  const { data } = req.body
  const { privateKey } = apiClient
  const signed = await signerService.signData(privateKey, data)
  res.send(signed)
}))

router.post('/apikeys/sign/tx', authenticate({ types: [AUTH_TYPE_API_KEYS] }), asyncHandler(async (req, res) => {
  const { apiClient } = req.securityContext
