...
const schemaFactory = () => ({
  ...
  agents: Joi.array().items(Joi.object().keys({
    ...
    is2FAConfigured: Joi.boolean().required(),
  ...
  static fromJson (data, context, options) {
    ...
    return new ProfileModel({
      ...
      agents: data.agents && data.agents.map(agent => ({
        ...
        clients: agent.clients.map(client => ({
          id: client.id,
          secret: client.secret,
          address: client.address,
          isReadAllowed: !!client.isReadAllowed,
          isMarketAllowed: !!client.isMarketAllowed,
          isLimitAllowed: !!client.isLimitAllowed,
          isWithdrawAllowed: !!client.isWithdrawAllowed
        }))
      }))
    })
  }
